...
mc_variability <- function(sensitivity, product, inv_C, C, res) {
  ...
  Variability <- matrix(NA, ncol = n_par, nrow = n_par) # Take care here, I find problems when using
  # Matrix classes in this step. I do not know why yet!!
  k4 <- res^4 - 3*diag(C)^2
  for(i in 1:n_par) {
    for(j in 1:n_par) {
